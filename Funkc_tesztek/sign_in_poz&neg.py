...
def sign_in(Email, Password):
    ...
    print(f'\nLog in +/- assert:'
          f'\nMenűelemek száma: {menusor_hossz}  '
          f'\n-->> user: {login_status[menusor_hossz]["user"]}, '
          f'\n     jogosultság: {login_status[menusor_hossz]["status"]}\n')
    ...
    if (menusor_hossz == 3): # sikertelen belépés
        ...
        msg_frame(f'Login - assert 1.: Sikertelen belépés - sikeres teszt negatív ágon - eredménye: "{signin_fail}"')
        ...
        msg_frame(f'Login + assert 1.-> igazolt belépés: A kilépés gomb lokalizálható!')
        ...
        # msg_frame(f'Login + assert 2.:{log_out_btn_txt} <=> {conf_text}: igazolt belépés: "Log out" szöveg megjelent!')
        msg_frame(f'Login + assert 2. -> igazolt belépés: {log_out_btn_txt} szöveg megjelent!')
