...
def sign_up(Username, Email, Password):
    ...
    sign_up_mnu_btn = browser.find_element(By.XPATH, '//a[@href="#/register"]')
    ...
    while reg_succ != 'Welcome!':
        ...
        sign_up_btn = browser.find_element(By.XPATH, '//button[@class="btn btn-lg btn-primary pull-xs-right"]')
        assert sign_up_btn.is_displayed()
        # if sign_up_btn.is_displayed():    # a belépéshez szükséges gomb meglétének ellenőrzése
        #     msg_frame('SIGN UP button assert -->  :)')
        sign_up_btn.click()
...
sign_up(Username_n[-1], Email_n[-1], 'Userpass1')
